...
{{--        </div>--}}
        ...
        @if (Route::has('reseller.password.request'))
            <p class="mb-1">
                <a href="{{ route('reseller.password.request') }}">{{ __('lang.' . strtoupper('forget_password')) }}</a>
            </p>
        @endif

        <p class="mb-0">
            <a href="{{ route('reseller.register') }}" class="text-center">{{ __('lang.' . strtoupper('register')) }}</a>
        </p>
    </div>
    <!-- /.login-card-body -->
@endsection
